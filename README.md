...
カテゴリが「その他」または空欄になっている商品を、既存のデータファイルだけを使って自動分類するスクリプトです。

`src/data/category_map.json` に定義された「カテゴリ → キーワード一覧」のマッピングをもとに、
商品名とのマッチングスコアを計算し、最もスコアの高いカテゴリを自動で割り当てます。
外部APIやLLMには依存せず、このリポジトリ内のデータだけで動作します。
...
- `src/data/category_map.json` 内のすべてのカテゴリ・キーワードと商品名をマッチング
- 出現回数 × キーワード長 に基づくスコアリングで最もスコアが高いカテゴリを採用
- スコアが0（どのカテゴリにもマッチしない）場合は「その他」を維持
...
│   ├── auto_categorizer.py    # 自己学習型カテゴリ分類スクリプト
